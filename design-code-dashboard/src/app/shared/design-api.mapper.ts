 ...
 * Production/Inventory arrays are aggregated into the single-object UI model.
 * ActivityTimeline maps 1:1 to timeline events.
 */
export function mapDesignDetail(dto: DesignDetailDto): DesignDetail {
  ...
  const productionRows = dto.production ?? [];
  const inventoryRows = dto.inventory ?? [];
  const timelineRows = dto.activityTimeline ?? [];

  const production = aggregateProduction(productionRows);
  const inventory = aggregateInventory(inventoryRows);
  ...
  const base: DesignListItem = {
    ...
    reservedQuantity: inventory.reservedStock,
    createdDate: '',
    designer: '',
  ...
  const timeline: TimelineEvent[] = timelineRows.map((e) => ({
    type: e.type ?? '',
    title: e.title ?? '',
    description: e.description ?? '',
    date: formatDisplayDate(e.date),
    icon: e.icon?.trim() || 'pi pi-circle',
    color: e.color?.trim() || '#64748b',
  }));
...
function aggregateProduction(rows: DesignProductionDto[]): DesignProductionInfo {
  if (!rows.length) {
    return {
  ...
  const first = rows[0];
  return {
    productionQuantity: rows.reduce((s, r) => s + (Number(r.productionQuantity) || 0), 0),
    completedQuantity: rows.reduce((s, r) => s + (Number(r.completedQuantity) || 0), 0),
    pendingQuantity: rows.reduce((s, r) => s + (Number(r.pendingQuantity) || 0), 0),
    rejectedQuantity: rows.reduce((s, r) => s + (Number(r.rejectedQuantity) || 0), 0),
    machine: first.machine?.trim() || rows.map((r) => r.machine?.trim()).find(Boolean) || '',
    department: first.department?.trim() || rows.map((r) => r.department?.trim()).find(Boolean) || '',
    supervisor: first.supervisor?.trim() || rows.map((r) => r.supervisor?.trim()).find(Boolean) || '',
  };
}

function aggregateInventory(rows: DesignInventoryDto[]): DesignInventoryInfo {
  if (!rows.length) {
    return {
      currentStock: 0,
      reservedStock: 0,
  ...
  const first = rows[0];
  return {
    currentStock: rows.reduce((s, r) => s + (Number(r.currentStock) || 0), 0),
    reservedStock: rows.reduce((s, r) => s + (Number(r.reservedStock) || 0), 0),
    availableStock: rows.reduce((s, r) => s + (Number(r.availableStock) || 0), 0),
    pendingStock: rows.reduce((s, r) => s + (Number(r.pendingStock) || 0), 0),
    warehouse: first.warehouse?.trim() || rows.map((r) => r.warehouse?.trim()).find(Boolean) || '',
    rack: first.rack?.trim() || rows.map((r) => r.rack?.trim()).find(Boolean) || '',
    location: first.location?.trim() || rows.map((r) => r.location?.trim()).find(Boolean) || '',
    batchNumber:
      first.batchNumber?.trim() || rows.map((r) => r.batchNumber?.trim()).find(Boolean) || '',
  };
}
