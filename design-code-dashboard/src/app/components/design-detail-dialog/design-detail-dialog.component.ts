...
export class DesignDetailDialogComponent implements OnInit {
  ...
  /** True when API returned production / inventory / timeline rows for this design. */
  ...
  ngOnInit(): void {
    ...
    this.designApi.getDesignById(designID, apiFilter).subscribe({
      next: (dto) => {
        ...
        this.hasProductionData.set((dto.production?.length ?? 0) > 0);
        this.hasInventoryData.set((dto.inventory?.length ?? 0) > 0);
        this.hasTimelineData.set((dto.activityTimeline?.length ?? 0) > 0);
        this.loading.set(false);
      },
      error: (err) => {
        this.loading.set(false);
